...
def search_event():
    ...
    organizersinfo = requests.post(app.config['USER_API'] + "/multipleuserinfo", data = json.dumps(organizeruserids)).json()
...
def get_events():
    ...
    userinfo = requests.get(app.config['USER_API'] + "/userinfo?id=" + str(organizerid)).json()
